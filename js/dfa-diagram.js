...
function renderDFADiagram(minDFA, containerEl) {
  ...
  // Sort edges by span so shorter ones get inner tracks to naturally prevent intersection
  edges.sort((a, b) => {
    const getSpan = (ed) => (ed.src !== deadId && ed.tgt !== deadId) 
      ? Math.abs(mainIds.indexOf(ed.tgt) - mainIds.indexOf(ed.src)) 
      : 0;
    return getSpan(a) - getSpan(b);
  });
  ...
  const highestArcY = maxTopTrack > 0 ? - (40 + maxTopTrack * ARC_STEP) : 0;
  const highestLoopY = -SR - (40 + maxSelfLoop * LOOP_STEP);
  ...
  const bottomArcY = bottomTracks.length > 0 ? maxBottomTrack * ARC_STEP : 0;
  ...
  // 7. Draw Edges
  // Pre-process: count edges converging to same target for spreading
  ...
  for (const e of edges) {
    ...
    if (e.type === 'self') {
      ...
      const height = 40 + e.track * ARC_STEP;
      let cx = (x1 + x2) / 2;
      let cy = Y_MAIN - 2 * height;
      
      // Spread control points for edges converging to same target
      const convergingEdges = edgesByTarget.get(`${e.tgt}`) || [];
      if (convergingEdges.length > 1) {
        const edgeIndex = convergingEdges.indexOf(e);
        const centerIndex = (convergingEdges.length - 1) / 2;
        const offset = edgeIndex - centerIndex;
        // Spread horizontally and vertically to create distinct approach angles
        cx += offset * 60;
        cy -= offset * 15;
      }
      ...
      if (e.src !== deadId) {
        const iSrc = mainIds.indexOf(e.src);
        const totalDeadEdges = edges.filter(ed => ed.type === 'dead' && ed.src !== deadId).length;
        // Spread control points so curves from different sources don't overlap
        const spreadFactor = (iSrc - (mainIds.length - 1) / 2) * 40;
        const cx = (x1 + x2) / 2 + spreadFactor;
        const cy = (Y_MAIN + Y_DEAD) / 2;
        const bz = getBezierEdge(x1, Y_MAIN, cx, cy, x2, Y_DEAD, SR + 4);

        path(`M ${bz.ex1} ${bz.ey1} Q ${cx} ${cy} ${bz.ex2} ${bz.ey2}`, stroke, mk);

        // Place label at t=0.35 along the curve, close to the source state
        const t = 0.35;
        labelX = (1 - t) * (1 - t) * bz.ex1 + 2 * (1 - t) * t * cx + t * t * bz.ex2;
        labelY = (1 - t) * (1 - t) * bz.ey1 + 2 * (1 - t) * t * cy + t * t * bz.ey2;

        text(labelX, labelY + 6, e.label, lbCol);
      }
    }
  }
  ...
  if (minDFA.states.has(minDFA.start)) {
    ...
    path(`M ${sx - SR - 60} ${Y_MAIN} L ${sx - SR - 10} ${Y_MAIN}`, '#3b82f6', MK_S);
    text(sx - SR - 35, Y_MAIN - 8, 'start', '#60a5fa');
  }
